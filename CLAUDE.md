...
For uploaded face-zone edits, use the documented belt in `docs/FACE_ZONE_EDIT_WORKFLOW.md` (see the `face-zone-edit-workflow` skill for the step-by-step belt). Do not substitute a rectangle mask, silently guess after semantic failure, or bypass the audited job route.
