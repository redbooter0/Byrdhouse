...
## 7. 2026-07-22 operating contract - final anime review

For hard-anime targets, `output_acceptance` proves framing only. The sidecar must also contain `likeness_acceptance`; when the anime face cannot be embedded, the candidate remains `needs_review` for founder comparison against the real Carey references. Never promote a result because it is brown, framed, or structurally clean.

The reviewed `--manual-box` ellipse is a maximum editable boundary. Parser recovery may subtract hair/headwear/accessories and refine skin inside it, but may not add pixels outside it. If a crop needs temporary top padding, the final composite is clamped to the immutable original width and height. `engine.ipadapter_weight` can be swept per job and is captured in the reproduce block.