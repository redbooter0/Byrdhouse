...
- 2026-07-22 - Image-lane structural closure on live RTX 3070: fixed canonical FINISH recipe replay, bounded reviewed masks, immutable-source padding/composite dimensions, per-job IP-Adapter weight, border fail-closed gate, and separate likeness acceptance. Vegeta/Luffy sweeps produced reproducible review candidates and a comparison sheet; full integration suite green. U1 visual closure remains open because stylized outputs cannot be machine-verified for Carey identity and therefore stay `needs_review` until founder approval.