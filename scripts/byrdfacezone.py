...
def _square_crop_box(points: np.ndarray, image_size: tuple[int, int], factor: float) -> tuple[int, int, int, int]:
    ...
    # A slight upward bias gives the forehead and hairline more room, which
    # matters for stylized anime heads that often sit high in frame.
    center_y -= (max_y - min_y) * 0.04
...
def _ordered_body_part_traversal(
    ...
    passed = (
        len(contour) >= 32
        and vertical_span >= 32
        and horizontal_span >= 32
        and neck_width >= 4
        and left[0] < top[0] < right[0]
        and top[1] < min(neck_left[1], neck_right[1])
    )
    return {
        "passed": bool(passed),
        "closed": True,
    ...
    stages = [
        {
            ...
            "passed": bool(
                detected_faces >= 1
                and (detection_score is None or detection_score >= 0.30)
                and mesh_points is not None
                and len(mesh_points) == 478
            ),
            "detected_faces": int(detected_faces),
            "detection_score": round(float(detection_score), 6) if detection_score is not None else None,
        ...
        },
        {
            ...
            "passed": bool(
                ...
                and float(mesh.get("coverage_ratio", 0.0)) >= 0.20
...
def _fit_target_mesh_jaw_to_semantic_outline(
    ...
    scale_x = float(np.clip(raw_ratio * 0.92, 1.0, 1.35))
    scale_y = float(min(1.15, 1.0 + (scale_x - 1.0) * 0.45))
    ...
    fitted[:, 1] = mesh_center_y + (fitted[:, 1] - mesh_center_y) * scale_y
    for axis in (0, 1):
    ...
    return fitted, {
        **base,
        ...
        "rule": "fit Carey jaw-to-jaw, then warp eyes/nose/mouth on the fitted 478-point grid",
    }
...
def prepare_face_zone(
    ...
    *,
    ...
    immutable_original = original.copy()
    ...
    ear_boxes = [] if manual_box is not None else _ear_boxes(full_face_points, mesh_points)
    ...
    if crop_box[1] <= 0:
        pad_top = 72
    if pad_top > 0:
        original = ImageOps.expand(original, border=(0, pad_top, 0, 0), fill=original.getpixel((0, 0)))
        crop_box = (crop_box[0], crop_box[1] + pad_top, crop_box[2], crop_box[3] + pad_top)
    source_crop_box = (
        crop_box[0], crop_box[1] - pad_top,
        crop_box[2], crop_box[3] - pad_top,
    )
    ...
    if manual_box is not None:
        # A founder-reviewed manual box is an elliptical recovery boundary,
        # never permission for the anime parser to flood the crop. Semantics
        # may refine/subtract inside it (hair/headwear still wins) but cannot
        # add pixels outside the reviewed face/head envelope.
        reviewed_limit = np.asarray(geometric_face.convert("L"))
        hard = Image.fromarray(
            np.minimum(np.asarray(hard.convert("L")), reviewed_limit), mode="L"
        )
        head_envelope = Image.fromarray(
            np.minimum(np.asarray(head_envelope.convert("L")), reviewed_limit), mode="L"
        )
        neck_anchor = Image.fromarray(
            np.minimum(np.asarray(neck_anchor.convert("L")), reviewed_limit), mode="L"
        )
    ...
    if np.any(head_boundary[0, :]):
        boundary_contacts.append("top")
    if np.any(head_boundary[-1, :]):
    ...
    immutable_original.save(original_path, "PNG", optimize=True)
    ...
    if identity_seed is not None and identity_warp is not None and identity_warp_mask is not None:
        ...
        # eye_source=target â€” inspectable on its own, not just a hint.
    ...
    yellow = Image.new("RGBA", immutable_original.size, (255, 220, 0, 0))
    ...
    ordered = immutable_original.convert("RGBA")
    full_head_mask = Image.new("L", immutable_original.size, 0)
    full_head_mask.paste(
        head_envelope.resize((right - left, bottom - top), Image.Resampling.NEAREST),
        (left, top),
    )
    full_hair_boundary = Image.new("L", immutable_original.size, 0)
    ...
    for mask, color in (
        ...
        ink = Image.new("RGBA", immutable_original.size, color)
    ...
    record = {
        ...
        "crop_preflight": {
            ...
            "rule": "expand and re-analyze any head/neck contour clipped by an available source edge before GPU",
        },
...
def composite_generated(zone_file: Path, generated_crop: Path, output: Path) -> Path:
    ...
    with Image.open(artifacts["soft_mask"]) as opened:
        ...
        # is the rectangular-crop-leak signature â€” clamp it and record the leak.
    ...
    box = zone["crop_box"]
    requested_width = int(box["width"])
    requested_height = int(box.get("height", requested_width))
    left, top = int(box["x"]), int(box["y"])
    right = min(original.width, left + requested_width)
    bottom = min(original.height, top + requested_height)
    visible_width = max(0, right - left)
    visible_height = max(0, bottom - top)
    if visible_width == 0 or visible_height == 0:
        raise RuntimeError("face-zone crop does not intersect the immutable source")

    def visible_layer(layer: Image.Image, resample: Image.Resampling) -> Image.Image:
        fitted = layer.resize((requested_width, requested_height), resample)
        return fitted.crop((0, 0, visible_width, visible_height))

    target_region = original.crop((left, top, right, bottom))
    generated = visible_layer(generated, Image.Resampling.LANCZOS)
    soft = visible_layer(soft, Image.Resampling.BILINEAR)
    if protected_color_mask is not None:
        protected_material = visible_layer(
            protected_color_mask, Image.Resampling.NEAREST
        )
    ...
    if hair_path and Path(hair_path).is_file():
        with Image.open(hair_path) as opened:
            hair = visible_layer(opened.convert("L"), Image.Resampling.BILINEAR)
        hair = hair.filter(ImageFilter.GaussianBlur(1.5))
        original.paste(target_region, (left, top), hair)

    # Repair 2026-07-16 (hard Vegeta) â€” three composite-time guards:
    ...
    if (eye_mask_path and Path(eye_mask_path).is_file()
        ...
        with Image.open(eye_mask_path) as opened:
            eye_mask = visible_layer(opened.convert("L"), Image.Resampling.BILINEAR)
    ...
    shard_report = {"shards_detected": None, "note": "no identity mesh â€” not applicable"}
    ...
    if not outside["passed"]:
        raise RuntimeError(
            ...
            f"({outside['changed_pixels']} px changed, max delta {outside['max_delta']}) â€” "
...
def acceptance_check(root: Path, image_path: Path,
    ...
    Never raises â€” any detection failure is recorded as a flag, not an exception.
    """
    ...
    if total > 1:
        result["flags"].append(f"multiple_faces: {total} detected â€” verify correct subject")
    if total == 0:
    ...
    if y1 / H < edge_clearance_pct:
        framing_flags.append("face_cropped_top â€” forehead may be cut off")
    if (H - y2) / H < edge_clearance_pct:
        framing_flags.append("face_cropped_bottom â€” chin may be cut off")
    if side < min_face_px:
